...
rna_models <- rna_models %>%
  ...
  mutate(regional_nl_model_result = map(data,
                                    ~gam(log2_rpkm ~
                                         ...
                                         #+ s(age, k=4, by=as.factor(cluster), bs='cs', fx=TRUE)
...
### CALCULATE CORTICAL AVERAGE TRAJECTORIES OVER TIME FOR EACH GENE  #####################################################################
cat('calculating gene trajectories modelled over time ....')

age_seq <- seq(from=80, to=260, length.out=50)
new_x = expand.grid(age=age_seq,
                    RIN=mean(residualised_rna_data$RIN),
                    sex='M',
                    sample='HSB103') #  sample is ignored anyway

trajectories<- rna_models %>% 
  group_by(symbol) %>%
  mutate(predicted = map(nl_model_result, ~predict(.x, newdata=new_x, exclude=c(s(sample)), se.fit=TRUE))) %>%
...
# add in age, cluster etc
all_new_x <- map_dfr(seq_len(length(unique(trajectories$symbol))), ~new_x)
trajectories <- cbind(all_new_x, trajectories)
cat('saving...')
write.csv(trajectories, 'results/gene-trajectories.csv', row.names = FALSE)
