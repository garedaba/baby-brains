...
def main(permute=False):
    ...
    for d, dataset in  enumerate(datasets):
        ...
        # for each bootstrapped gene sample, fit a model to predict PC-correlation from age
        ...
        prenatal_model_fits, prenatal_r2, x_range = calculate_model_fits(correlation_data)
        ...
        print('see: results/PC1-correlations-' + datanames[d] + '-model-fits.csv')
        model_fit_df.to_csv('results/PC1-correlations-' + datanames[d] + '-model-fits.csv', index=None)
        ...
        print('see: results/PC1-correlations-' + datanames[d] + '-model-results.csv')
        model_results.to_csv('results/PC1-correlations-' + datanames[d] + '-model-results.csv', index=None)
        print("")
