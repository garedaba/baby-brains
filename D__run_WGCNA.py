...
for pos_or_neg in ['positive', 'negative']:
    ...
    if pos_or_neg=='positive':
        sns.heatmap(order_data, cmap='Reds', vmin=0, linecolor='white', linewidths=.1, square=True, xticklabels=True, yticklabels=True)
    else:
        sns.heatmap(order_data, cmap='Blues', vmin=0, linecolor='white', linewidths=.1, square=True, xticklabels=True, yticklabels=True)

    plt.savefig('graphs/WCGNA_' + pos_or_neg + '_gene_coexpression_matrix.pdf')
    print("see: graphs/WCGNA_" + pos_or_neg + "_gene_coexpression_matrix.pdf")
